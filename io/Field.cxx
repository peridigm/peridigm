 ...
 */
FieldSpec::FieldSpec(FieldType t, FieldLength len, const string& label) : type(t), length(len), label(label){}
FieldSpec::FieldSpec(const FieldSpec& c) : type(c.getType()), length(c.getLength()), label(c.getLabel()) {}
bool FieldSpec::operator == (const FieldSpec& right) const {
	return (type == right.getType() && length == right.getLength()) ? true : false;
}
bool FieldSpec::operator != (const FieldSpec& right) const {
	return !(*this==right);
}
