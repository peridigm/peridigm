...
test_dir = "Compression_QS_Explicit_MultiFreqOutput_3x2x2/np2"
base_name = "Compression_QS_Explicit_MultiFreqOutput_3x2x2"
...
if __name__ == "__main__":
    ...
    command = ["../../../../scripts/epu", "-p", "2", base_name+"_QS"]
    p = Popen(command, stdout=logfile, stderr=logfile)
    return_code = p.wait()
    if return_code != 0:
        result = return_code
    command = ["../../../../scripts/epu", "-p", "2", base_name+"_Explicit"]
    p = Popen(command, stdout=logfile, stderr=logfile)
    return_code = p.wait()
    if return_code != 0:
        result = return_code
