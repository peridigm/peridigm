/*
 * utPdImp_implicitLinearDynamicsDemo_bar.cpp
 *
...
/**
 * Cross section of cylinder is in x-y plane
 * Axis of cylinder is along z-axis
 */
...
)
{
	...
	/*
	 * Get points for initial conditions
	 */
	PdNeighborhood::CoordinateLabel axis = PdNeighborhood::Z;

	/*
	 * Left end of cylinder
	 */
	Pd_shared_ptr_Array<int> leftIds = PdNeighborhood::getPointsInNeighborhoodOfAxisAlignedMinimumValue(axis,pdGridData.myX,pdGridData.numPoints,horizon,zMin);
	/*
	 * Right end of cylinder
	 */
	Pd_shared_ptr_Array<int> rightIds = PdNeighborhood::getPointsInNeighborhoodOfAxisAlignedMaximumValue(axis,pdGridData.myX,pdGridData.numPoints,horizon,zMax);

	/*
	 * Ends of cylinder should have same number of points -- but only in serial
	 */
	if(1==numProcs)
		BOOST_CHECK(leftIds.getSize() == rightIds.getSize());
	 ...
	 */
	{
		 ...
		 * Points on left
		 */
		int *leftIdPtr = leftIds.get();
		int id;
		double *vPtr = velocity.getField(NP1).getArray().get();
		for(std::size_t p=0;p<leftIds.getSize();p++,leftIdPtr++){
			/*
			 * Left END: points initially moving to the left
			 */
			id  = *leftIdPtr;
			*(vPtr+3*id+2)=INITIAL_VELOCITY;
		}
	}

	{
		 ...
		 * Points on right
		 */
		int *rightIdPtr = rightIds.get();
		int id;
		double *vPtr = velocity.getField(NP1).getArray().get();
		for(std::size_t p=0;p<rightIds.getSize();p++,rightIdPtr++){
			/*
			 * Right END: points initially moving to the right
			 */
			id = *rightIdPtr;
			*(vPtr+3*id+2)=-INITIAL_VELOCITY;
		}
...
void runWave() {
	...
	PdGridData pdGridData =  PdQuickGrid::getDiscretization(myRank, meshGen);
	pdGridData = getLoadBalancedDiscretization(pdGridData);
	pdGridData = createAndAddNeighborhood(pdGridData,horizon);
	...
	PdVTK::CollectionWriter collectionWriter("utPdImp_implicitLinearDynamicsDemo_bar",comm.NumProc(), comm.MyPID(), PdVTK::vtkASCII);
	 ...
	 */
	std::size_t numStepsPerPeriod = 50;
	/*
	 * nT -- eigen function number
	 */
	std::size_t numPeriods = 1;
	int periodNumber = 1;
	double omega = periodNumber*M_PI*sqrt(E/rho)/(2.0*cylinderLength);
...
bool init_unit_test_suite()
{
  ...
  test_suite* proc = BOOST_TEST_SUITE("utPdImp_implicitLinearDynamicsDemo_bar");
